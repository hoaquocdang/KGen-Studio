...
async function loadPromptLibrary() {
    ...
    try {
        ...
        statusText.textContent = 'Ðang t?i thu vi?n prompt...';

        const response = await fetch('./data/trending-prompts.json');
        barFill.style.width = '60%';
        statusText.textContent = 'Ðang x? lý d? li?u...';
        ...
        statusText.textContent = `Ðã t?i ${APP_STATE.prompts.length.toLocaleString()} prompts!`;
        ...
        document.getElementById('gallery-subtitle').textContent =
            `${APP_STATE.prompts.length.toLocaleString()} prompt du?c tuy?n ch?n t? c?ng d?ng sáng t?o`;
        ...
        showToast('Không th? t?i thu vi?n prompt', 'error');
...
function setupGalleryEvents() {
    ...
    document.getElementById('btn-shuffle').addEventListener('click', () => {
        ...
        renderGallery(true);
        showToast('?? Ðã xáo tr?n th? t?!', 'info');
    });
...
function setupModal() {
    ...
    document.getElementById('btn-use-prompt').addEventListener('click', () => {
        if (!isLoggedIn()) {
            showToast('?? Vui lòng dang nh?p d? s? d?ng prompt', 'error');
            openAuthModal();
            return;
        }
        ...
        closeModal();
        switchTab('generate');
        showToast('? Prompt dã du?c chuy?n sang tab T?o ?nh', 'success');
    });
...
function setupGenerateEvents() {
    ...
    document.getElementById('btn-enhance-from-gen').addEventListener('click', () => {
        ...
        if (!prompt.trim()) {
            showToast('Vui lòng nh?p prompt tru?c', 'error');
            return;
        }
    ...
    document.getElementById('btn-use-as-ref')?.addEventListener('click', () => {
        ...
        if (img.src) {
            ...
            renderRefPreviews();
            showToast('? Ðã thêm vào ?nh tham chi?u', 'success');
        }
    });
}
...
function updateCharCount() {
    ...
    document.getElementById('prompt-char-count').textContent = `${textarea.value.length} ký t?`;
}
...
function addReferenceImage() {
    ...
    input.value = '';
    renderRefPreviews();
    showToast('? Ðã thêm ?nh tham chi?u', 'success');
}
...
async function generateImage() {
    ...
    if (!prompt) {
        showToast('Vui lòng nh?p prompt', 'error');
        return;
    }

    // Check login
    if (!isLoggedIn()) {
        showToast('?? Vui lòng dang nh?p d? t?o ?nh', 'error');
        openAuthModal();
        return;
    }
    ...
    if (!canGenerateImage()) {
        const plan = getUserPlan();
        const limit = getUserImageLimit();
        showToast(`?? B?n dã s? d?ng h?t ${limit} ?nh mi?n phí. Nâng c?p gói Pro d? ti?p t?c!`, 'error', 5000);
        switchTab('pricing');
        return;
    }
    ...
    try {
        ...
        if (result && result.imageUrl) {
            ...
            showToast('?? ?nh dã du?c t?o thành công!', 'success');
        } else {
            throw new Error('Không nh?n du?c ?nh t? server. Vui lòng th? l?i.');
        }
    } catch (error) {
        clearInterval(timerInterval);
        document.getElementById('result-loading').classList.add('hidden');
        ...
        const errorMsg = error.message || 'L?i không xác d?nh';
...
function showGenerationError(message) {
    ...
    if (lower.includes('token') || lower.includes('api key') || lower.includes('c?u hình')) {
        guidance = '?? Vui lòng vào Cài d?t (thanh bên) d? thêm API token KGen ho?c OpenAI key.';
    } else if (lower.includes('401') || lower.includes('403') || lower.includes('unauthorized')) {
        guidance = '?? API token không h?p l? ho?c dã h?t h?n. Ki?m tra l?i trong Cài d?t.';
    } else if (lower.includes('402') || lower.includes('credit') || lower.includes('insufficient')) {
        guidance = '?? H?t credit. Credit mi?n phí s? du?c reset m?i ngày.';
    } else if (lower.includes('429') || lower.includes('rate')) {
        guidance = '? Quá nhi?u request. Vui lòng d?i vài giây r?i th? l?i.';
    } else if (lower.includes('timeout') || lower.includes('timed out')) {
        guidance = '?? Request quá h?n. Th? l?i — có th? server dang b?n.';
    } else if (lower.includes('network') || lower.includes('fetch') || lower.includes('econnrefused')) {
        guidance = '?? L?i k?t n?i m?ng. Ki?m tra internet và th? l?i.';
    } else if (lower.includes('safety') || lower.includes('policy') || lower.includes('flagged')) {
        guidance = '?? Prompt có th? vi ph?m chính sách n?i dung. Th? ch?nh s?a prompt.';
    } else {
        guidance = '?? Th? l?i ho?c ch?n model/provider khác.';
    }
...
async function generateViaComfyUI(prompt, negativePrompt) {
    ...
    if (!url) throw new Error('ComfyUI URL chua du?c c?u hình');
...
async function generateViaOpenAI(prompt, model, quality, size, negativePrompt) {
    ...
    if (!apiKey) throw new Error('OpenAI API Key chua du?c c?u hình');
...
async function generateViaKGen(prompt, model, aspectRatio) {
    ...
    if (!token) throw new Error('KGen Token chua du?c c?u hình. Vào Cài d?t d? thêm token.');
    ...
    if (!data.success || !data.generationId) {
        throw new Error(data.error || 'Không nh?n du?c generation ID t? server');
    }
...
async function pollKGenGeneration(generationId, token, timeoutMs = 300000) {
    ...
    while (Date.now() - startTime < timeoutMs) {
        ...
        if (!statusRes.ok) {
            throw new Error(`L?i ki?m tra tr?ng thái: ${statusRes.status}`);
        }
        ...
        if (status.status === 'failed') {
            throw new Error(status.error || 'Generation th?t b?i trên server');
        }
    ...
    throw new Error('Generation quá th?i gian ch? (5 phút). Vui lòng th? l?i.');
}
...
async function generateViaKGen(prompt, model, aspectRatio) {
    ...
    if (!response.ok) {
        if (response.status === 401 || response.status === 403) {
            throw new Error('C?n API token d? t?o ?nh. Vào Cài d?t ? KGen Cloud d? thêm token, ho?c dang ký t?i KGen.ai');
        }
        const err = await response.json().catch(() => ({}));
        throw new Error(err.error || `KGen error: ${response.status}`);
    }
    ...
    const data = await response.json();
    if (!data.success || !data.generationId) {
        throw new Error(data.error || 'Không nh?n du?c generation ID');
    }
...
async function pollKGenGeneration(generationId, baseUrl, timeoutMs = 300000) {
    ...
    throw new Error('Generation quá th?i gian ch?. Vui lòng th? l?i.');
}
...
function setupEnhanceEvents() {
    ...
    document.getElementById('btn-use-enhanced').addEventListener('click', () => {
        ...
        updateCharCount();
        switchTab('generate');
        showToast('? Prompt nâng c?p dã s?n sàng d? t?o ?nh!', 'success');
    });
}
...
function enhancePrompt() {
    ...
    if (!input) {
        showToast('Vui lòng nh?p ý tu?ng', 'error');
        return;
    }
    ...
    showToast('? Prompt dã du?c nâng c?p!', 'success');
}
...
async function checkComfyUIConnection() {
    ...
    statusText.textContent = 'Ðang ki?m tra...';
    ...
    try {
        ...
        if (response.ok) {
            ...
            showToast('? Ðã k?t n?i ComfyUI!', 'success');
        ...
        statusText.textContent = 'Không th? k?t n?i. Hãy d?m b?o ComfyUI dang ch?y.';
        showToast('? Không th? k?t n?i ComfyUI', 'error');
    }
}
...
function handleWorkflowFile(file) {
    ...
    reader.onload = (e) => {
        try {
            ...
            renderWorkflowList();
            showToast(`? Ðã import workflow "${name}"`, 'success');
        } catch (err) {
            showToast('JSON không h?p l?', 'error');
        }
    };
        ...
        return `
            <div class="form-card" style="margin-bottom: 12px;">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    ...
                    <button class="btn btn-sm btn-ghost" onclick="deleteWorkflow('${escapeHtml(name)}')">??? Xoá</button>
                </div>
            </div>
        `;
    }).join('');
...
function deleteWorkflow(name) {
    ...
    localStorage.setItem('kgen_workflows', JSON.stringify(workflows));
    renderWorkflowList();
    showToast(`??? Ðã xoá workflow "${name}"`, 'info');
}
...
function saveSettings() {
    ...
    showToast('?? Cài d?t dã du?c luu!', 'success');
}
...
function resetSettings() {
    ...
    updateProviderStatus();
    showToast('?? Ðã khôi ph?c cài d?t m?c d?nh', 'info');
}
...
function toggleAuthMode() {
    ...
    if (authMode === 'register') {
        ...
        title.textContent = 'T?o Tài Kho?n';
        subtitle.textContent = 'Ðang ký mi?n phí d? truy c?p toàn b? prompts';
        switchText.textContent = 'Ðã có tài kho?n?';
        switchBtn.textContent = 'Ðang nh?p';
    } else {
        loginForm.classList.remove('hidden');
        registerForm.classList.add('hidden');
        title.textContent = 'Ðang Nh?p';
        subtitle.textContent = 'Ðang nh?p d? m? khóa toàn b? thu vi?n 1,300+ prompts';
        switchText.textContent = 'Chua có tài kho?n?';
        switchBtn.textContent = 'Ðang ký';
    }
}
...
function openAuthModal() {
    ...
    loginForm.classList.remove('hidden');
    registerForm.classList.add('hidden');
    document.getElementById('auth-title').textContent = 'Ðang Nh?p';
    document.getElementById('auth-subtitle').textContent = 'Ðang nh?p d? m? khóa toàn b? thu vi?n 1,300+ prompts';
    document.getElementById('auth-switch-text').textContent = 'Chua có tài kho?n?';
    document.getElementById('btn-switch-auth').textContent = 'Ðang ký';
...
function handleRegister() {
    ...
    if (!name || !email || !password) {
        showToast('Vui lòng di?n d?y d? thông tin', 'error');
        return;
    }
    if (password.length < 6) {
        showToast('M?t kh?u ph?i có ít nh?t 6 ký t?', 'error');
        return;
    }
    ...
    if (users[email]) {
        showToast('Email này dã du?c dang ký', 'error');
        return;
    }
    ...
    showToast(`?? Chào m?ng ${name}! Tài kho?n dã du?c t?o thành công`, 'success');
}
...
function handleLogin() {
    ...
    if (!email || !password) {
        showToast('Vui lòng nh?p email và m?t kh?u', 'error');
        return;
    }
    ...
    if (!user) {
        showToast('Email không t?n t?i. Hãy dang ký tài kho?n m?i.', 'error');
        return;
    }

    if (user.passwordHash !== btoa(password)) {
        showToast('M?t kh?u không dúng', 'error');
        return;
    }
    ...
    showToast(`?? Chào m?ng tr? l?i, ${user.name}!`, 'success');
}
    ...
    banner.innerHTML = `
        <div class="guest-banner-text">
            ...
            <div>
                ...
                <p>Ch? hi?n th? <strong>10%</strong> n?i dung prompt. Ðang nh?p mi?n phí d? xem d?y d?!</p>
            </div>
        </div>
        <button class="btn btn-primary btn-sm" id="btn-guest-login">?? Ðang nh?p ngay</button>
    `;
...
function handleGoogleSignIn() {
    ...
    if (!googleClientInitialized) {
        ...
        initGoogleSignIn();
        if (!googleClientInitialized) {
            showToast('Google Sign-In dang kh?i t?o, th? l?i sau giây lát...', 'info');
            return;
        }
    }
    ...
    try {
        google.accounts.id.prompt((notification) => {
            if (notification.isNotDisplayed()) {
                ...
                showToast('Vui lòng cho phép popup t? Google', 'info');
            }
        ...
        showToast('L?i Google Sign-In. Vui lòng th? l?i.', 'error');
    }
}
...
function handleGoogleCredentialResponse(response) {
    try {
        ...
        if (!payload || !payload.email) {
            showToast('Không th? xác th?c v?i Google', 'error');
            return;
        }
        ...
        showToast(`?? Chào m?ng ${googleUser.name}! Ðã dang nh?p b?ng Google`, 'success');
    } catch (error) {
        console.error('Google credential error:', error);
        showToast('L?i x? lý thông tin Google', 'error');
    }
}
...
function showGoogleSetupPrompt() {
    ...
    infoDiv.innerHTML = `
        <div class="google-setup-content">
            ...
            <p>Ð? s? d?ng dang nh?p Google, b?n c?n:</p>
            <ol>
                ...
                <li>Dán Client ID vào <strong>Cài d?t ? Google Client ID</strong></li>
            </ol>
            <div style="display:flex;gap:8px;margin-top:12px">
                <button class="btn btn-primary btn-sm" id="btn-goto-settings-google">?? M? Cài d?t</button>
                <button class="btn btn-ghost btn-sm" id="btn-close-google-setup">Ðóng</button>
            </div>
        </div>
    `;
            ...
            ${PRICING_TIERS.map(tier => `
                <div class="pricing-card-inline ${tier.popular ? 'popular' : ''} ${currentTier === tier.id ? 'current' : ''}">
                    ...
                    <button class="btn ${tier.buttonClass} tier-btn-inline" 
                        ...
                        ${currentTier === tier.id ? '\u2713 Ðang s? d?ng' : tier.buttonText}
                    </button>
                </div>
            `).join('')}
        </div>
        <div class="pricing-footer-inline">
            <p>\ud83d\udcb3 Thanh toán an toàn qua <strong>Stripe</strong> \u2022 H?y b?t c? lúc nào</p>
            <p class="sub-note">H? tr?: MoMo, Visa, Mastercard, JCB \u2022 Hoàn ti?n 7 ngày</p>
        </div>
    `;
...
function updateGuideSlide() {
    ...
    if (nextBtn) {
        nextBtn.textContent = guideCurrentSlide === GUIDE_TOTAL_SLIDES - 1 ? 'B?t d?u dùng! ??' : 'Ti?p ?';
    }
}
    ...
    if (describeCard) {
        describeCard.addEventListener('click', () => {
            ...
            input.onchange = (e) => {
                ...
                if (!file) return;
                showToast('Ðang phân tích ?nh...', 'info');
                // For demo, just show that we received the file
                setTimeout(() => {
                    showToast('Tính nang Describe Image s? t? d?ng t?o prompt t? ?nh', 'info');
                }, 1500);
            };
            input.click();
        });
